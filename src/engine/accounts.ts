import type { Account, AllocationCategoryDef, Scenario } from '../types'
...
export function sumBalances(accounts: Account[], filter: (a: Account) => boolean): number {
  return accounts.filter(filter).reduce((sum, a) => sum + a.balance, 0)
}
...
export function getAllocationBreakdown(
  ...
  const assetAccounts = accounts.filter((a) => !a.isLiability && a.balance > 0)
  const map = new Map<string, number>()

  for (const account of assetAccounts) {
    const cat = account.allocationCategory
    map.set(cat, (map.get(cat) ?? 0) + account.balance)
  }
...
export function computeDashboardMetrics(
  ...
  const netWorth =
    assets.reduce((s, a) => s + a.balance, 0) - liabilities.reduce((s, a) => s + a.balance, 0)
  ...
  const debt = liabilities.reduce((s, a) => s + a.balance, 0)
...
export function accountsToCsv(accounts: Account[]): string {
  const headers = [
    ...
    'taxTreatment', 'employerMatchPercent', 'contributionIncreaseRate', 'allocationCategory', 'isLiability', 'notes',
  ]
