...
export function AllocationPieChart({
  ...
  const renderLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }: {
    ...
    return (
      <text x={x} y={y} fill="currentColor" className="text-ledger-text" textAnchor="middle" dominantBaseline="central" fontSize={11}>
        ...
        <PieChart>
          ...
          <Tooltip
            ...
            formatter={(value: number, name: string) => {
              ...
              return [`${formatCurrency(value, currency)} (${formatPercent(pct)})`, name]
            }}
          />
        </PieChart>
      </ResponsiveContainer>
        ...
        {data.map((entry) => {
          ...
          return (
            <li key={entry.label} className="flex items-center justify-between text-sm">
              ...
              <span className="tabular-nums text-ledger-muted">
                {formatCurrency(entry.value, currency)} · {formatPercent(pct)}
              </span>
            </li>
          )
        })}
